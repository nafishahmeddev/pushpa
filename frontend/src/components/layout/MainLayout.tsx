...
export default function MainLayout() {
  ...
  return (
    <div className="h-dvh grid grid-cols-[250px_1fr] grid-rows-1 bg-gray-100">
      <div className="bg-white border-e h-full grid grid-rows-[60px_1fr_70px]">
        ...
        <div className="px-2 h-full py-2">
          <div className="bg-gray-200 rounded-full items-center flex p-0.5 gap-2 pe-2 w-full h-full">
            ...
            <div className="flex items-start justify-center flex-col flex-1">
              ...
              <span className="text-xs">{auth.user.designation}</span>
            </div>

            <button
              className=" text-red-700 px-2 hover:opacity-50"
              onClick={() => {
                AuthApi.logout().then(() => {
                  navigate("/");
                });
