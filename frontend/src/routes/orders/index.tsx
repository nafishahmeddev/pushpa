...
export const Route = createFileRoute("/orders/")({ component: RouteComponent });
              ...
              {result.records.map((order, index: number) => {
                return (
                  <TableRow key={`product-${order.id}`}>
                    ...
                    <TableCell className="px-0 py-2 w-0 sticky left-0 bg-white">
                      <div className="inline-flex flex-nowrap gap-2 text-gray-600 px-2">
                        ...
                        {[OrderStatus.Draft, OrderStatus.Ongoing].includes(order.status) && (
                          <Link className={`hover:opacity-50`} to={"/pos/" + order.id} title="Go to billing">
                    ...
                    <TableCell className="text-nowrap text-end">{order.status != OrderStatus.Cancelled && <>{order.kotList?.map((e) => <button className="inline-flex mx-0.5 bg-lime-600 rounded-lg h-5 px-1 min-w-5 items-center justify-center text-white text-xs" key={e.id}>{e.tokenNo}</button>)}</>}</TableCell>
