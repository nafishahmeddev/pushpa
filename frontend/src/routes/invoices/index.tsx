...
export const Route = createFileRoute("/invoices/")({ component: RouteComponent });
                ...
                <TableRow key={`product-${invoice.id}`}>
                  ...
                  <TableCell className="px-0 py-2 w-0 sticky left-0 bg-white">
                    <div className="inline-flex flex-nowrap gap-2 text-gray-600 px-2">
                      <button className={`hover:opacity-50`} onClick={() => handleOnDetails(invoice.id)}>
                        <Icon icon="ph:receipt" height={20} width={20} />
                      </button>

                      <button className={`hover:opacity-50 text-lime-700`} onClick={() => handleOnDetails(invoice.id, true)}>
                        <Icon icon="lsicon:print-outline" height={20} width={20} />
                      </button>
                    </div>
                  </TableCell>
                  <TableCell>{invoice.receiptNo}</TableCell>
                  <TableCell className="text-nowrap">{Formatter.datetime(invoice.createdAt)}</TableCell>
                  <TableCell className="text-end">{Formatter.money(invoice.tax)}</TableCell>
                  <TableCell className="text-end">{Formatter.money(invoice.amount)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
